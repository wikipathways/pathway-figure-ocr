...
## READ DF
df.table <- readRDS("pfocr_covid_table.rds")
df.years <- readRDS("pfocr_covid_years.rds")
df.genes <- readRDS("pfocr_covid_genes.rds")
df.annots <- readRDS("pfocr_covid_annots.rds")

# df.active <<- df.table
# df.active.genes <<- df.genes %>% filter(figid %in% df.active$figid)

ui <- fixedPage(
  titlePanel("COVID-19 Pathway Figures"),
  sidebarLayout(
    sidebarPanel(
        useShinyjs(), 
        h4("Introduction"),
...
server <- function(input, output, session) {
  ...
  output$table <- DT::renderDataTable(
    DT::datatable(df.reactive.table()[,c('pmcid','paper.title','authors','year','number','figure.title' )],
                  ...
                  filter = 'top',
                  rownames= FALSE,
                  selection = list(mode = 'single', selected = c(184)), #top row sorted by desc(year)

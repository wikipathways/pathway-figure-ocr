...
fetch.path <- "~/Dropbox (Gladstone)/Pathway Figure OCR/INDRA text mining"
image.path <- paste(fetch.path, "shiny", sep = '/')
...
setwd(fetch.path)
df.all <- readRDS("pfocr_figures.rds") ## MOD: df.target.rds or df.all.rds
df.all <- df.all %>%
  arrange(-(as.numeric(pathway_score))) %>%
  # top_n(-100) %>%

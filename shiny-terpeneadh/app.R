## Terpene/ADH Pathway Filter/Search/View
...
readPfocrData <- function(session,df.table,df.years,df.genes,df.chems ){
  withProgress(message = 'Loading data...', detail = "",
               ...
               value = NULL, {
                 df.table <<- readRDS("pfocr_terpeneadh_table.rds")
                 df.years <<- readRDS("pfocr_terpeneadh_years.rds")
                 df.genes <<- readRDS("pfocr_terpeneadh_genes.rds")
                 df.chems <<- readRDS("pfocr_terpeneadh_chems.rds")
               })
}
...
ui <- fixedPage(
  titlePanel("Terpene/ADH Pathway Figures"),
  sidebarLayout(
    sidebarPanel(
        useShinyjs(), 
        h4("Introduction"),
        HTML('The <a href="https://www.wikipathways.org/index.php/WikiPathways:Team#Team_Members" target="_blank">WikiPathways team</a>
          at <a href="https://gladstone.org/people/alex-pico" target="_blank">Gladstone Institutes</a> identified a subset of terpene and ADH related pathway figures
...
server <- function(input, output, session) {
  ...
  output$table <- DT::renderDataTable(
    ...
    DT::datatable(df.reactive.table()[,c('pmcid','paper.title','authors','year','number','figure.title' )],
                  ...
                  filter = 'top',
                  rownames= FALSE,
                  selection = list(mode = 'single', selected = c(615)), #top row sorted by desc(year)

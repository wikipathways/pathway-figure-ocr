...
readPfocrData <- function(session,df.table,df.years,df.genes,df.chems ){
  withProgress(message = 'Loading data...', detail = "",
               ...
               value = NULL, {
                 df.table <<- readRDS("pfocr_terpeneplant_table.rds")
                 df.years <<- readRDS("pfocr_terpeneplant_years.rds")
                 df.genes <<- readRDS("pfocr_terpeneplant_genes.rds")
                 df.chems <<- readRDS("pfocr_terpeneplant_chems.rds")
               })
}
...
ui <- fixedPage(
  titlePanel("Terpene and Other Plant Pathway Figures"),
  sidebarLayout(
    sidebarPanel(
        useShinyjs(), 
        h4("Introduction"),
